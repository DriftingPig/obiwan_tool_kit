...
ylim = 0.3
major_ticks = np.arange(0,ylim,0.02)
minor_ticks = np.arange(0, ylim, 0.01)
...
obiwan = plt.errorbar(ran_eboss_dat[0],ran_eboss_dat[1],ran_eboss_dat[2],label = 'obiwan')
Johan = plt.errorbar(Johan_random[0],Johan_random[1],Johan_random[2],label = 'Johan')
uniform = plt.errorbar(uniform_random[0],uniform_random[1],uniform_random[2],label = 'uniform')
weighed = plt.errorbar(weighed_random[0],weighed_random[1],weighed_random[2],label = 'weighed')
plt.legend()
plt.xlabel(r'$\theta$'+'(Degree)')
plt.ylabel(r'w('+r'$\theta$'+')')
plt.title('eBoss chunk23')
plt.gca().set_xscale("log", nonposx='clip')
from matplotlib.backends.backend_pdf import PdfPages
with PdfPages('./plots/validation3.pdf') as pdf:
	pdf.savefig() 
plt.show()
